...
export default function LastNamesAllUpdateForm(props) {
  const {
    id: idProp,
    lastNamesAll,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = lastNamesAllRecord
      ? { ...initialValues, ...lastNamesAllRecord }
      : initialValues;
    setAll_last(cleanValues.all_last);
    setPersian_last(cleanValues.persian_last);
    setArab_last(cleanValues.arab_last);
    setKurmanji_last(cleanValues.kurmanji_last);
    setPakistani_last(cleanValues.pakistani_last);
    setSorani_last(cleanValues.sorani_last);
    setTurkish_last(cleanValues.turkish_last);
    setUkrainian_last(cleanValues.ukrainian_last);
    setGerman_last(cleanValues.german_last);
    setErrors({});
  };
  const [lastNamesAllRecord, setLastNamesAllRecord] =
    React.useState(lastNamesAll);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(LastNamesAll, idProp)
        : lastNamesAll;
      setLastNamesAllRecord(record);
    };
    queryData();
  }, [idProp, lastNamesAll]);
  React.useEffect(resetStateValues, [lastNamesAllRecord]);
  const validations = {
    all_last: [],
    persian_last: [],
    arab_last: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            LastNamesAll.copyOf(lastNamesAllRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "LastNamesAllUpdateForm")}
      {...rest}
    >
      <TextField
        label="All last"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || lastNamesAll)}
